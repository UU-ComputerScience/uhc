...
addEnvNamedElems hpt vs
  = let (low, high) = bounds hpt
        ...
        e2 = listArray (low, high+extra) (replaceIn 0 (elems hpt ++ replicate extra AbsBottom) vs)
    in e2
    
replaceIn :: Int -> [a] -> [(Int,a)] -> [a]
replaceIn i xs [] = xs
replaceIn i (x:xs) rs@((j,y):ys) = if i == j then y:replaceIn (i+1) xs ys
                                             else x:replaceIn (i+1) xs rs
