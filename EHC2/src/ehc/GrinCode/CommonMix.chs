...
addEnvNamedElems hpt vs
  = let (low, high) = bounds hpt
        ...
        e2 = listArray (low, high+extra) (replaceIn (elems hpt ++ replicate extra AbsBottom) vs)
    in e2
    
replaceIn :: Int -> [a] -> [(Int,a)] -> [a]
replaceIn i xs [] = xs
replaceIn i (x:xs) [(i,y):ys] = y:replaceIn (i+1) xs ys
replaceIn i (x:xs) rs = x:replaceIn (i+1) xs rs
