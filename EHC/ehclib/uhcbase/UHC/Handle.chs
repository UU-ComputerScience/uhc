...
%%[doesWhat doclatex
A Handle represents a file.
The implementation is here is a stripped down version from the one in GHC.IOBase,
see there for documentation, with the following differences to be kept in mind:
\begin{itemize}
\item No concurrency is offered in UHC.
\item Use of MVar's is left but is only there to minimize the differences with GHC.
\item DuplexHandle for read/write streams is left in the code, but is (as of 20090416) not offered.
\end{itemize}

As of 20090412:
A Handle is defined in Prelude as well, hidden here, to be sorted out when the implementation here
is functioning well.
%%]
...
withHandle' fun h m act =
   ...
   (h',v)  <- (act h_ `catchAny` \err -> putMVar m h_ >> throw err)
              `catch` \ex -> ioError (augmentIOError ex fun h)
...
readCharFromBuffer slab off
  = IO $ \s -> case readUnicodeCharArray slab off s of 
                 ( s', (c,n) ) -> s' `seq` ( s', ( c,  off +  n) )
{-
...
foreign import ccall unsafe "unistd.h read"
   readInto :: CInt -> Ptr CChar -> CSsize -> IO CSsize

foreign import ccall unsafe "unistd.h write"
   writeFrom :: CInt -> Ptr CChar -> CSsize -> IO CSsize

read_off :: CInt -> Ptr CChar -> Int -> CSsize -> IO CSsize
read_off fd ptr off sz = readInto fd (ptr `plusPtr` off) sz

write_off :: CInt -> Ptr CChar -> Int -> CSsize -> IO CSsize
write_off fd ptr off sz = writeFrom fd (ptr `plusPtr` off) sz

read_rawBuffer :: CInt -> RawBuffer -> Int -> CSsize -> IO CSsize
read_rawBuffer fd b off sz = read_off fd (mutableByteArrayPtr b) off sz

write_rawBuffer :: CInt -> RawBuffer -> Int -> CSsize -> IO CSsize
write_rawBuffer fd b off sz = write_off fd (mutableByteArrayPtr b) off sz
...
safe_read_off :: FD -> Ptr CChar -> Int -> CSsize -> IO CSsize
safe_read_off = read_off

safe_write_off :: FD -> Ptr CChar -> Int -> CSsize -> IO CSsize
safe_write_off = write_off

safe_read_rawBuffer :: FD -> RawBuffer -> Int -> CSsize -> IO CSsize
safe_read_rawBuffer = read_rawBuffer

safe_write_rawBuffer :: FD -> RawBuffer -> Int -> CSsize -> IO CSsize
safe_write_rawBuffer = write_rawBuffer
...
fdToHandle' fd mb_type is_socket filepath mode binary
 = do
       let mb_stat = case mb_type of
                        ...
                        Just other       -> Just (other,0::CDev,0::CIno)
...
foreign import ccall unsafe "HsBase.h __hscore_bufsiz"   dEFAULT_BUFFER_SIZE :: Int
