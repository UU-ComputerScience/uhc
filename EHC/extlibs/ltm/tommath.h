...
/* define heap macros */
   ...
   #define XMALLOC(x)  		NULL
   #define XFREE(x)    		return MP_UHC
   #define XREALLOC(x,y) 	NULL
   #define XCALLOC(x,y)  	return MP_UHC
#else
#ifndef CRYPT
...
#if __UHC_BUILDS_RTS__
#define MP_UHC        -4  /* UHC does not allow allocation */
#endif
...
typedef int ltm_prime_callback(unsigned char *dst, int len, void *dat);
	...
	#define SIGN(m)    			GB_LTM_Int_Sign(m)
	
	#define SET_ALLOC(m,v)		return MP_UHC
	#define SET_DIGITS(m,v)		return MP_UHC
#else
	#define ALLOC(m)   			((m)->alloc)
	#define USED(m)    			((m)->used)
	#define DIGITS(m)  			((m)->dp)
	#define SIGN(m)    			((m)->sign)
	
	#define SET_ALLOC(m,v)		ALLOC(m) = (v)
	#define SET_DIGITS(m,v)		DIGITS(m) = (v)
#endif

#define DIGIT(m,k) 			(DIGITS(m)[(k)])

#define SET_USED(m,v)		USED(m) = (v)
#define SET_SIGN(m,v)		SIGN(m) = (v)
#define SET_DIGIT(m,k,v)	DIGIT(m,k) = (v)
#define OR_DIGIT(m,k,v)		DIGIT(m,k) |= (v)
#define AND_DIGIT(m,k,v)	DIGIT(m,k) &= (v)
#define XOR_DIGIT(m,k,v)	DIGIT(m,k) ^= (v)
#define INC_DIGIT(m,k,v)	DIGIT(m,k) += (v)

#define INC_USED(m,v)		USED(m) += (v)
#define INC1_USED(m)		USED(m)++
...
#define s_mp_mul(a, b, c) s_mp_mul_digs(a, b, c, USED(a) + USED(b) + 1)
