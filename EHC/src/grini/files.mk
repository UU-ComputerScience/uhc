...
GRINI_BLD_EXEC							:= $(EHC_BIN_VARIANT_ASPECTS_PREFIX)$(GRINI_EXEC_NAME)$(EXEC_SUFFIX)
...
GRINI_HS_MAIN_DRV_HS					:= $(patsubst $(SRC_GRINI_PREFIX)%.chs,$(EHC_BLD_VARIANT_ASPECTS_PREFIX)%.hs,$(GRINI_HS_MAIN_SRC_CHS))

GRINI_HS_UTIL_SRC_CHS					:= $(patsubst %,$(SRC_GRINI_PREFIX)%.chs,GRINICommon GRINIRun)
GRINI_HS_UTIL_DRV_HS					:= $(patsubst $(SRC_GRINI_PREFIX)%.chs,$(EHC_BLD_VARIANT_ASPECTS_PREFIX)%.hs,$(GRINI_HS_UTIL_SRC_CHS))
...
# derived
GRINI_AG_S_MAIN_DRV_AG					:= $(patsubst $(SRC_GRINI_PREFIX)%.cag,$(EHC_BLD_VARIANT_ASPECTS_PREFIX)%.ag,$(GRINI_AG_S_MAIN_SRC_CAG))
...
grini-variant-dflt: $(GRINI_HS_ALL_DRV_HS) $(GRINI_AG_ALL_MAIN_DRV_HS) $(LIB_EHC_INS_FLAG)
	mkdir -p $(dir $(GRINI_BLD_EXEC))
	$(GHC) --make $(GHC_OPTS) $(GHC_OPTS_OPTIM) -package $(LIB_EHC_PKG_NAME) -i$(EHC_BLD_VARIANT_ASPECTS_PREFIX) $(EHC_BLD_VARIANT_ASPECTS_PREFIX)$(GRINI_MAIN).hs -o $(GRINI_BLD_EXEC)

$(GRINI_AG_ALL_MAIN_DRV_AG) $(GRINI_AG_ALL_DPDS_DRV_AG): $(EHC_BLD_VARIANT_ASPECTS_PREFIX)%.ag: $(SRC_GRINI_PREFIX)%.cag $(SHUFFLE)
	mkdir -p $(@D)
...
$(GRINI_AG_S_MAIN_DRV_HS): %.hs: %.ag
	$(AGC) -cfspr $(UUAGC_OPTS_WHEN_EHC) -P$(EHC_BLD_VARIANT_ASPECTS_PREFIX) -P$(INSTALLFORBLD_EHC_LIB_AG_PREFIX) $<

$(GRINI_HS_MAIN_DRV_HS): $(EHC_BLD_VARIANT_ASPECTS_PREFIX)%.hs: $(SRC_GRINI_PREFIX)%.chs $(SHUFFLE)
	mkdir -p $(@D)
	$(SHUFFLE_HS) $(LIB_EHC_SHUFFLE_DEFS) --gen-reqm="($(EHC_VARIANT) $(EHC_ASPECTS))" --base=Main --variant-order="$(EHC_SHUFFLE_ORDER)" $< > $@

$(GRINI_HS_UTIL_DRV_HS): $(EHC_BLD_VARIANT_ASPECTS_PREFIX)%.hs: $(SRC_GRINI_PREFIX)%.chs $(SHUFFLE)
	mkdir -p $(@D)
