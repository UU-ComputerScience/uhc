...
infer2pass-variant-dflt: $(INF2PS_ALL_DPDS) $(LIB_EH_UTIL_INS_FLAG)
	mkdir -p $(dir $(INF2PS_BLD_EXEC))
	$(GHC) --make $(GHC_OPTS) -package $(LIB_EH_UTIL_PKG_NAME) -i$(INF2PS_BLD_VARIANT_PREFIX) $(INF2PS_BLD_VARIANT_PREFIX)$(INF2PS_MAIN).hs -o $(INF2PS_BLD_EXEC)
...
$(INF2PS_AG_D_MAIN_DRV_HS): $(INF2PS_BLD_VARIANT_PREFIX)%.hs: $(INF2PS_BLD_VARIANT_PREFIX)%.ag
	mkdir -p $(@D) ; \
	$(AGC) -dr -P"$(SRC_INF2PS_PREFIX)" -P"$(INF2PS_BLD_VARIANT_PREFIX)" -o $@ $<

$(INF2PS_AG_S_MAIN_DRV_HS): $(INF2PS_BLD_VARIANT_PREFIX)%.hs: $(INF2PS_BLD_VARIANT_PREFIX)%.ag
	mkdir -p $(@D) ; \
	$(AGC) -cfspr -P"$(SRC_INF2PS_PREFIX)" -P"$(INF2PS_BLD_VARIANT_PREFIX)" -o $@ $<

$(INF2PS_AG_DS_MAIN_DRV_HS): $(INF2PS_BLD_VARIANT_PREFIX)%.hs: $(INF2PS_BLD_VARIANT_PREFIX)%.ag
	mkdir -p $(@D) ; \
	$(AGC) -dcfspr -P"$(SRC_INF2PS_PREFIX)" -P"$(INF2PS_BLD_VARIANT_PREFIX)" -o $@ $<
