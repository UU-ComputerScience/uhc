...
GRINC_BLD_EXEC							:= $(EHC_BIN_VARIANT_PREFIX)$(GRINC_EXEC_NAME)
GRINC_ALL_PUB_EXECS						:= $(patsubst %,$(EHC_BIN_PREFIX)%/grinc,$(GRIN_PUB_VARIANTS))
GRINC_ALL_EXECS							:= $(patsubst %,$(EHC_BIN_PREFIX)%/grinc,$(GRIN_VARIANTS))
...
grinc-variant-dflt: $(GRINC_ALL_HS_SRC) $(LIB_EH_UTIL_INS_FLAG) $(LIB_EHC_INS_FLAG)
	...
	$(GHC) --make $(GHC_OPTS) -package $(LIB_EH_UTIL_PKG_NAME) -package $(LIB_EHC_PKG_NAME) -i$(EHC_BLD_VARIANT_PREFIX) $(GRINC_MAIN_HS) -o $(GRINC_BLD_EXEC)

# variant dispatch rules
$(patsubst $(BIN_PREFIX)%$(EXEC_SUFFIX),%,$(GRINC_ALL_EXECS)): %: $(BIN_PREFIX)%$(EXEC_SUFFIX)

$(GRINC_ALL_EXECS): %: $(GRINC_ALL_SRC)
	$(MAKE) EHC_VARIANT=$(notdir $(*D)) grinc-variant-$(notdir $(*D))
