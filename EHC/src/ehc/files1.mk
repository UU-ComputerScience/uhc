...
# library
# derived stuff
LIB_EHC_CABAL_DRV						:= $(EHC_BLD_LIBEHC_VARIANT_PREFIX)lib-$(GHC_PKG_NAME_PREFIX)$(LIB_EHC_BASE)$(EHC_VARIANT).cabal
...
EHC_HS_UTIL_SRC_CHS						:= $(patsubst %,$(SRC_EHC_PREFIX)%.chs,\
													...
													Base/Opts Base/BasicAnnot Base/Common Base/Builtin Base/Builtin2 Base/HsName Base/Debug Base/Trie Base/CfgPP Base/ForceEval \
													NameAspect \
...
EHC_SHUFFLE_OPTS_WHEN_UHC_100			:= $(SHUFFLE_OPTS_WHEN_UHC)
...
$(EHC_AG_ALL_MAIN_DRV_AG) $(EHC_AG_ALL_DPDS_DRV_AG): $(EHC_BLD_LIB_HS_VARIANT_PREFIX)%.ag: $(SRC_EHC_PREFIX)%.cag $(SHUFFLE)
	mkdir -p $(@D)
	$(SHUFFLE_AG) $(LIB_EHC_SHUFFLE_DEFS) $(SHUFFLE_OPTS_WHEN_EHC) $(EHC_SHUFFLE_OPTS_WHEN_UHC_$(EHC_VARIANT)) --gen=$(EHC_VARIANT) --base=$(*F)  --order="$(EHC_SHUFFLE_ORDER)" $< > $@&& \
	touch $@
