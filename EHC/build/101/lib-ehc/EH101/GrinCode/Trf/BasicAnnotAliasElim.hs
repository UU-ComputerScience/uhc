...
-- UUAGC 0.9.39.1 (build/101/lib-ehc/EH101/GrinCode/Trf/BasicAnnotAlia)
module EH101.GrinCode.Trf.BasicAnnotAliasElim(grBasicAnnotAliasElim) where
...
sem_GrAlt_Alt ann_ pat_ expr_  =
    (case (Set.empty) of
     { _exprOunitNmBoundByEval ->
     (case (expr_ _exprOunitNmBoundByEval ) of
      { ( _exprItrf,_exprIunitUnwrapMp) ->
          (case (pat_ ) of
           { ( _patItrf) ->
...
{-
   visit 0:
      inherited attribute:
         unitNmBoundByEval    : FvS
      synthesized attributes:
         trf                  : SELF 
         unitUnwrapMp         : UnitUnwrapMp
   alternatives:
      alternative App:
         child nm             : {HsName}
         child argL           : GrValL 
...
sem_GrExpr_Catch body_ arg_ handler_  =
    (\ _lhsIunitNmBoundByEval ->
         (case (_lhsIunitNmBoundByEval) of
          { _handlerOunitNmBoundByEval ->
          (case (_lhsIunitNmBoundByEval) of
           { _bodyOunitNmBoundByEval ->
           (case (handler_ _handlerOunitNmBoundByEval ) of
            { ( _handlerItrf,_handlerIunitUnwrapMp) ->
                (case (body_ _bodyOunitNmBoundByEval ) of
                 { ( _bodyItrf,_bodyIunitUnwrapMp) ->
                     (case (GrExpr_Catch _bodyItrf arg_ _handlerItrf) of
                      { _trf ->
                      (case (_trf) of
                       { _lhsOtrf ->
                       (case (_bodyIunitUnwrapMp `Map.union` _handlerIunitUnwrapMp) of
                        { _lhsOunitUnwrapMp ->
                        ( _lhsOtrf,_lhsOunitUnwrapMp) }) }) }) }) }) }) }))
         ...
         (case (_lhsIunitNmBoundByEval) of
          ...
          (case (pat_ ) of
           { ( _patItrf) ->
               (case (expr_ _exprOunitNmBoundByEval ) of
                { ( _exprItrf,_exprIunitUnwrapMp) ->
                    (case (case (_exprItrf,_patItrf) of
                             (GrExpr_Eval _, GrPatLam_Var n) -> Set.singleton n
                             _ -> Set.empty) of
                     { _newBnd ->
                     (case (Set.union _newBnd _lhsIunitNmBoundByEval) of
                      { _bodyOunitNmBoundByEval ->
                      (case (body_ _bodyOunitNmBoundByEval ) of
                       { ( _bodyItrf,_bodyIunitUnwrapMp) ->
                           (case (GrExpr_Seq _exprItrf _patItrf _bodyItrf) of
                            { _trf ->
                            (case (case _exprItrf of
                                     GrExpr_Unit (GrVal_Var n) _ | Set.member n _lhsIunitNmBoundByEval
                                       -> case _patItrf of
                                            GrPatLam_BasicNode  _ _ -> Map.singleton n _patItrf
                                            GrPatLam_EnumNode   _   -> Map.singleton n _patItrf
                                            GrPatLam_PtrNode    _   -> Map.singleton n _patItrf
                                            GrPatLam_OpaqueNode _   -> Map.singleton n _patItrf
                                            _ -> Map.empty
                                     _ -> Map.empty) of
                             { _newMp ->
                             (case (if not (Set.null _newBnd)
                                    then maybe _trf (\b -> GrExpr_Seq _exprItrf b _bodyItrf) (Map.lookup (Set.findMin _newBnd) _bodyIunitUnwrapMp)
                                    else if not (Map.null _newMp)
                                    then _bodyItrf
                                    else _trf) of
                              { _lhsOtrf ->
                              (case (Map.union _newMp _bodyIunitUnwrapMp) of
                               { _lhsOunitUnwrapMp ->
                               ( _lhsOtrf,_lhsOunitUnwrapMp) }) }) }) }) }) }) }) }) }) }))
