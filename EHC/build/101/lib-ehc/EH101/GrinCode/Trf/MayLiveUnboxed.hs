...
-- UUAGC 0.9.39.1 (build/101/lib-ehc/EH101/GrinCode/Trf/MayLiveUnboxed)
module EH101.GrinCode.Trf.MayLiveUnboxed(grMayLiveUnboxed) where
...
{-
   visit 0:
      inherited attribute:
         tagAllowsUnboxedLife : GrTag -> Unbox
...
-- semantic domain
type T_GrAGItf  = (GrTag -> Unbox) ->
                  ( GrModule )
data Inh_GrAGItf  = Inh_GrAGItf {tagAllowsUnboxedLife_Inh_GrAGItf :: !((GrTag -> Unbox))}
data Syn_GrAGItf  = Syn_GrAGItf {trf_Syn_GrAGItf :: !(GrModule )}
wrap_GrAGItf :: T_GrAGItf  ->
                Inh_GrAGItf  ->
                Syn_GrAGItf 
wrap_GrAGItf sem (Inh_GrAGItf _lhsItagAllowsUnboxedLife )  =
    (let ( _lhsOtrf) = sem _lhsItagAllowsUnboxedLife 
     in  (Syn_GrAGItf _lhsOtrf ))
sem_GrAGItf_AGItf :: T_GrModule  ->
                     T_GrAGItf 
sem_GrAGItf_AGItf module_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _moduleOtagAllowsUnboxedLife ->
          (case (module_ _moduleOtagAllowsUnboxedLife ) of
           { ( _moduleItrf) ->
               (case (_moduleItrf) of
                { _lhsOtrf ->
                ( _lhsOtrf) }) }) }))
...
sem_GrAdapt_Del off_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _offOtagAllowsUnboxedLife ->
          (case (off_ _offOtagAllowsUnboxedLife ) of
           { ( _offItrf) ->
...
sem_GrAdapt_Ins off_ val_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _valOtagAllowsUnboxedLife ->
          (case (_lhsItagAllowsUnboxedLife) of
           { _offOtagAllowsUnboxedLife ->
           (case (val_ _valOtagAllowsUnboxedLife ) of
            { ( _valItrf) ->
                (case (off_ _offOtagAllowsUnboxedLife ) of
                 { ( _offItrf) ->
...
sem_GrAdaptL_Cons hd_ tl_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _tlOtagAllowsUnboxedLife ->
          (case (_lhsItagAllowsUnboxedLife) of
           { _hdOtagAllowsUnboxedLife ->
           (case (tl_ _tlOtagAllowsUnboxedLife ) of
            { ( _tlItrf) ->
                (case (hd_ _hdOtagAllowsUnboxedLife ) of
                 { ( _hdItrf) ->
...
sem_GrAlt_Alt ann_ pat_ expr_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _exprOtagAllowsUnboxedLife ->
          (case (_lhsItagAllowsUnboxedLife) of
           { _patOtagAllowsUnboxedLife ->
           (case (expr_ _exprOtagAllowsUnboxedLife ) of
            { ( _exprItrf) ->
                (case (pat_ _patOtagAllowsUnboxedLife ) of
                 { ( _patItrf) ->
...
sem_GrBind_Rec bindL_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _bindLOtagAllowsUnboxedLife ->
          (case (bindL_ _bindLOtagAllowsUnboxedLife ) of
           { ( _bindLItrf) ->
...
sem_GrExpr_App nm_ argL_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _argLOtagAllowsUnboxedLife ->
          (case (argL_ _argLOtagAllowsUnboxedLife ) of
           { ( _argLItrf) ->
...
sem_GrExpr_Case val_ altL_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _altLOtagAllowsUnboxedLife ->
          (case (_lhsItagAllowsUnboxedLife) of
           { _valOtagAllowsUnboxedLife ->
           (case (altL_ _altLOtagAllowsUnboxedLife ) of
            { ( _altLItrf) ->
                (case (val_ _valOtagAllowsUnboxedLife ) of
                 { ( _valItrf) ->
...
sem_GrExpr_Catch body_ arg_ handler_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _handlerOtagAllowsUnboxedLife ->
          (case (_lhsItagAllowsUnboxedLife) of
           { _bodyOtagAllowsUnboxedLife ->
           (case (handler_ _handlerOtagAllowsUnboxedLife ) of
            { ( _handlerItrf) ->
                (case (body_ _bodyOtagAllowsUnboxedLife ) of
                 { ( _bodyItrf) ->
          ...
          (case (val_ _valOtagAllowsUnboxedLife ) of
           { ( _valItrf) ->
               (case (GrExpr_Store _valItrf) of
                { _trf ->
                (case (case _valItrf of
                         GrVal_Node _ _        -> _trf
                         GrVal_NodeAdapt _ _   -> _trf
                         v                     -> GrExpr_Unit v GrType_None) of
                 { _lhsOtrf ->
                 ( _lhsOtrf) }) }) }) }))
...
sem_GrExpr_Unit val_ type_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _valOtagAllowsUnboxedLife ->
          (case (_lhsItagAllowsUnboxedLife) of
           { _typeOtagAllowsUnboxedLife ->
           (case (type_ _typeOtagAllowsUnboxedLife ) of
            { ( _typeItrf) ->
                (case (val_ _valOtagAllowsUnboxedLife ) of
                 { ( _valItrf) ->
...
sem_GrModule_Mod moduleNm_ globalL_ bindL_ tagsMp_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _bindLOtagAllowsUnboxedLife ->
          (case (_lhsItagAllowsUnboxedLife) of
           { _globalLOtagAllowsUnboxedLife ->
           (case (bindL_ _bindLOtagAllowsUnboxedLife ) of
            { ( _bindLItrf) ->
                (case (globalL_ _globalLOtagAllowsUnboxedLife ) of
                 { ( _globalLItrf) ->
...
sem_GrPatAlt_Node tag_ fldL_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _tagOtagAllowsUnboxedLife ->
          (case (tag_ _tagOtagAllowsUnboxedLife ) of
           { ( _tagItrf) ->
...
sem_GrPatAlt_NodeSplit tag_ nm_ fldL_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _fldLOtagAllowsUnboxedLife ->
          (case (fldL_ _fldLOtagAllowsUnboxedLife ) of
           { ( _fldLItrf) ->
               (case (_lhsItagAllowsUnboxedLife) of
                { _tagOtagAllowsUnboxedLife ->
                (case (tag_ _tagOtagAllowsUnboxedLife ) of
          ...
          (case (fldL_ _fldLOtagAllowsUnboxedLife ) of
           { ( _fldLItrf) ->
               (case (GrPatLam_VarNode _fldLItrf) of
                { _trf ->
                (case (case _fldLItrf of
                         [GrVar_KnownTag t, GrVar_Var v]  -> case _lhsItagAllowsUnboxedLife t of
...
sem_GrType_Arrow args_ res_  =
    (\ _lhsItagAllowsUnboxedLife ->
         (case (_lhsItagAllowsUnboxedLife) of
          { _resOtagAllowsUnboxedLife ->
          (case (res_ _resOtagAllowsUnboxedLife ) of
           { ( _resItrf) ->
               (case (_lhsItagAllowsUnboxedLife) of
                { _argsOtagAllowsUnboxedLife ->
                (case (args_ _argsOtagAllowsUnboxedLife ) of
                 { ( _argsItrf) ->
...
-- semantic domain
type T_GrTypeBaseL  = (GrTag -> Unbox) ->
                      ( GrTypeBaseL )
                ...
                (case (tag_ _tagOtagAllowsUnboxedLife ) of
                 { ( _tagItrf) ->
                     (case (GrVal_Node _tagItrf _fldLItrf) of
                      { _trf ->
                      (case (case (_fldLItrf,_lhsItagAllowsUnboxedLife _tagItrf) of
                               ([f],Unbox_FirstField)
                                 -> f
                               ([],Unbox_Tag t)
                                 -> GrVal_LitInt t
