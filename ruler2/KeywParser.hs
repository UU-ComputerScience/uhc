...
pAllParsers
  = let alg                 =   (undefined,sem_Expr_App,sem_Expr_AppTop)
        ...
        pMbViewSel          =   pMaybe sem_ViewSel_All id (pKey "viewsel" *> pViewSel)
        ...
        pNmV                =   Nm <$> pVarid
        pNmVw               =   pNm' pNmStrI
        pSymEscStr             =   pVarsym <|> pConsym
                            <|> pAnyKey keywordsOpsEsc
                            <|> pKey "`"  *> (   (\n -> "`" ++ n ++ "`") <$> pNmStr
                                             <|> concat <$> pList1 (pAnyKey keywordsOpsParenEsc)
                                             )
                                         <*  pKey "`"
        pSym                =   Nm <$> pSymEscStr
        ...
        pExprExplain        =   foldl1 sem_Expr_SP <$> pList1 pW
                            where pW =   pExprAtom (Nm <$> (pNmStr <|> pSymEscStr <|> pAnyKey (keywordsOpsExplainEsc ++ keywordsText)))
        ...
        pExprSpecial        =   (sem_Expr_StrAsIs . concat)     <$> pList1 (pAnyKey (keywordsOpsParenEsc ++ keywordsTextEscapable))
