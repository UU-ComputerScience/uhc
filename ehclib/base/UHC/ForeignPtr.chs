...
--
-- Adapted for use in UHC.
-- Current notable differences: minimal finalizers because Boehm GC is used, which does not move allocated mem
-- 
-----------------------------------------------------------------------------

-- #hide
module UHC.ForeignPtr
  (
        ForeignPtr(..),
        FinalizerPtr,
        FinalizerEnvPtr,
...
addForeignPtrConcFinalizer_ (PlainForeignPtr r) finalizer = do
  ...
  if noFinalizers
     then case r of
            IORef (STRef mr) -> do
                  _ <- mkWeak mr () (Just (foreignPtrFinalizer r))
                  return ()
     else return ()
addForeignPtrConcFinalizer_ f@(MallocPtr fo r) finalizer = do
  noFinalizers <- noMixing HaskellFinalizers r (return finalizer)
  if noFinalizers
     then do _ <- mkWeak fo () (Just (do foreignPtrFinalizer r; touch f))
             return ()
     else return ()
