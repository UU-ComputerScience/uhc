...
SHUFFLE_C			:= $(SHUFFLE) --plain --preamble=no --lhs2tex=no --line=no
...
$(SHUFFLE_BLD_EXEC): $(SHUFFLE_AG_ALL_MAIN_DRV_HS) $(SHUFFLE_HS_ALL_DRV_HS) $(LIB_EH_UTIL_INS_FLAG)
	$(GHC) --make $(GHC_OPTS) $(GHC_OPTS_OPTIM) -package $(LIB_EH_UTIL_PKG_NAME) -i$(SHUFFLE_BLD_PREFIX) $(SHUFFLE_BLD_PREFIX)$(SHUFFLE_MAIN).hs -o $@
	$(STRIP) $@

$(SHUFFLE_AG_D_MAIN_DRV_HS): $(SHUFFLE_BLD_PREFIX)%.hs: $(SRC_SHUFFLE_PREFIX)%.ag
	mkdir -p $(@D) ; \
	$(AGC) --module=$(*F) -dr -P$(SRC_SHUFFLE_PREFIX) -o $@ $<

$(SHUFFLE_AG_S_MAIN_DRV_HS): $(SHUFFLE_BLD_PREFIX)%.hs: $(SRC_SHUFFLE_PREFIX)%.ag
	mkdir -p $(@D) ; \
	$(AGC) -cfspr -P$(SRC_SHUFFLE_PREFIX) -o $@ $<

$(SHUFFLE_AG_DS_MAIN_DRV_HS): $(SHUFFLE_BLD_PREFIX)%.hs: $(SRC_SHUFFLE_PREFIX)%.ag
	mkdir -p $(@D) ; \
	$(AGC) --module=$(*F) -dcfspr -P$(SRC_SHUFFLE_PREFIX) -o $@ $<
