...
GRINI_HS_MAIN_DRV_HS					:= $(patsubst $(SRC_GRINI_PREFIX)%.chs,$(GRIN_BLD_VARIANT_PREFIX)%.hs,$(GRINI_HS_MAIN_SRC_CHS))

GRINI_HS_UTIL_SRC_CHS					:= $(patsubst %,$(SRC_GRINI_PREFIX)%.chs,GRICommon GRIParser GRIRun)
GRINI_HS_UTIL_DRV_HS					:= $(patsubst $(SRC_GRINI_PREFIX)%.chs,$(GRIN_BLD_VARIANT_PREFIX)%.hs,$(GRINI_HS_UTIL_SRC_CHS))
...
$(patsubst $(SRC_GRINI_PREFIX)%.cag,$(GRIN_BLD_VARIANT_PREFIX)%.hs,$(GRINI_AGSETUP_MAIN_SRC_CAG)) \
										: $(patsubst $(SRC_GRIN_PREFIX)%.cag,$(GRIN_BLD_VARIANT_PREFIX)%.ag,$(GRIN_AGSETUP_DPDS_SRC_CAG))
...
# derived
GRINI_AG_S_MAIN_DRV_AG					:= $(patsubst $(SRC_GRINI_PREFIX)%.cag,$(GRIN_BLD_VARIANT_PREFIX)%.ag,$(GRINI_AG_S_MAIN_SRC_CAG))
