...
%%[8
void mm_traceSupply_GBModule_Init( MM_TraceSupply* traceSupply, MM_Malloc* memmgt, MM_Trace* trace ) {
	MM_TraceSupply_GBModule_Data* trgr = memmgt->malloc( sizeof(MM_TraceSupply_GBModule_Data) ) ;
	trgr->trace = trace ;
...
void mm_traceSupply_GBModule_Run( MM_TraceSupply* traceSupply ) {
	MM_TraceSupply_GBModule_Data* trgr = (MM_TraceSupply_GBModule_Data*)traceSupply->data ;
	...
	for ( i = 0 ; i < mm_flexArray_SizeUsed(gbMods) ; i++ ) {
		...
		for ( j = 0 ; j < gbMod->nrCafGlobalEntryIndices ; j++ ) {
			...
			*e = mm_Trace_TraceObject( trgr->trace, *e ) ;
