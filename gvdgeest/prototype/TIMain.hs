...
tiExpr ce as (Var i)          = do sc         <- find i as
                                   (ps :=> t) <- freshInst sc Prove
                                   return (ps, t)
tiExpr ce as (Const (i:>:sc)) = do (ps :=> t) <- freshInst sc Prove
                                   return (ps, t)
...
-----------------------------------------------------------------------------
-- ce = class and instance environment
-- fs = the set of fixed type variables
-- gs = the set of type vars over wich we want to quantify
-- ps = the list of predicates
--
-- 1. reduce the predicates to HNF
-- 2. split the ps in ps that only contain fixed type-vars, and the rest
-- 3. defaulting
-- 4. (remaining predicates, predicates of this gen-point)
...
tiExpl ce as (i, sc, alts)
        = do (qs :=> t) <- freshInst sc Assume    -- inst of sign
             ps         <- tiAlts ce as alts t    -- inference alternatives
             s          <- getSubst               -- get subst.
             let qs'     = apply s qs             -- apply subst on signature preds
                 t'      = apply s t              -- apply subst on signature type
                 fs      = tv (apply s as)        -- apply subst on Assumptions, and obtain the type variables 
                 gs      = tv t' \\ fs            -- get only the free type-variables from the type       
                 sc'     = strip $ quantify gs (qs':=>t') -- quanity the type-signature again
             (ds,rs)    <- split ce fs gs (qs' ++ (apply s ps))
             if sc /= sc' then 
...
strip (Forall ks (ps :=> t)) = Forall ks ((map stripPred ps) :=> t)

stripPred (Assume p _) = p
stripPred (Prove  p _) = p
